...
class SeeActAgent:
    def __init__(self,
        ...
        self.main_path = os.path.join(self.config["basic"]["save_file_dir"], cat, task_id)
...
ELEMENT: The uppercase letter of your choice.''',
    ...
    async def predict(self):
        ...
        choices = choices[:-1]
        try: 
            elDex_list = range(min(len(choices), 26))
            elDex = random.choice(elDex_list)

            pred_element_label = chr(ord('@')+elDex + 1)
            pred_action = "CLICK"
            pred_value = None
            ...
            prediction={"element": pred_element,
    ...
    async def execute(self, prediction_dict):
        ...
        # if self.config['playwright']['tracing']:
        #     # Need this or playwright files will just always be blank screens
        #     try:
        #         await self.session_control["active_page"].wait_for_load_state('load')
        #     except Exception as e:
        #         pass
