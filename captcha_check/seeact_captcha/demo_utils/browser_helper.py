...
async def get_element_description(element, tag_name, role_value, type_value):
    ...
    '''
    # text_content = await element.inner_text(timeout=2000)
    ...
    if num_parents > 0:
        ...
        parent_text = (await parent_locator.inner_text(timeout=2000) or "").strip()
        if parent_text:
    ...
    if tag_name == "select":
        ...
        text2 = await element.evaluate(
            "select => select.options[select.selectedIndex].textContent", timeout=2000
        )
        ...
        if text2:
            ...
            if not text4:
                text4 = await element.text_content(timeout=2000)
                if not text4:
                    text4 = await element.inner_text(timeout=2000)
    ...
    if tag_name == "input" or tag_name == "textarea":
        if role_value not in none_input_type and type_value not in none_input_type:
            text1 = "input value="
            text2 = await element.input_value(timeout=2000)
            if text2:
    ...
    if text:
        ...
        if len(text) > 80:
            text_content_in = await element.inner_text(timeout=2000)
    ...
    for attr in salient_attributes:
        attribute_value = await element.get_attribute(attr, timeout=2000)
        if attribute_value:
    ...
    if num_childs>0:
        for attr in salient_attributes:
            attribute_value = await first_child_locator.get_attribute(attr, timeout=2000)
            if attribute_value:
                text1 += f"{attr}=" + "\"" + attribute_value.strip() + "\"" + " "
...
async def get_element_data(element, tag_name,viewport_size,seen_elements=[]):
    try:
        ...
        if await element.is_hidden(timeout=2000) or await element.is_disabled(timeout=2000):
            return None
        ...
        if tag_name in tag_name_list:
            ...
            real_tag_name = await element.evaluate("element => element.tagName.toLowerCase()", timeout=2000)
        ...
        role_value = await element.get_attribute('role', timeout=2000)
        type_value = await element.get_attribute('type', timeout=2000)
