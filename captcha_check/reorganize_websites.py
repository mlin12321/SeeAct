...
def main():
    with open('captcha_check/data/website_save_list.txt', 'r') as f:
        web_data = f.read()
    ...
    for datapiece in web_data:
        ...
        try:
            todo_flag = datapiece[3].strip().replace(" ", "_")
        except:
            pass
    ...
    with open('captcha_check/data/website_list.txt', 'w') as f:
        for cat in cat_dict:
            for web_data in cat_dict[cat]:
                website = web_data[0].strip().replace(" ", "_")
                cat = web_data[1].strip().replace(" ", "_")
                url = web_data[2].strip().replace(" ", "_")
